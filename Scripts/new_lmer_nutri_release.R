...
concen_quick=function(x){
  res00=lmerTest::lmer(Concentration~Period*Access+Location+(1|Position),
...
amount_quick=function(x){
  res01=lmerTest::lmer(ResidAmount~Period*Access+Location+(1|Location/Position),
                       nutri_stat %>% filter(Nutrient==x))
...
ratio_quick=function(x){
  res02=lmerTest::lmer(nutri_ratio[[x]]~Period*Access+Location+(1|Location/Position),nutri_ratio)
  print(sjPlot::plot_model(res02,'int')+
          labs(title=x,y=paste0(x,' in residual litter, mg per sample'))+
          theme_test())
  print(paste('====================',x,'ratio======================'))
  print(anova(res02))
  print(summary(res02))
